...
function DateTimeCard({city}) {
  ...
  return (
    <article className="flex flex-col justify-center items-center p-8 w-full max-w-lg bg-white rounded-2xl shadow-lg text-gray-900 border border-gray-300">
      ...
      <time className="mt-4 text-6xl text-gray-900 font-extrabold">{new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time> 
      <time className="mt-2 text-lg text-gray-500">{new Date().toDateString()}</time>
    </article>
  );
}
